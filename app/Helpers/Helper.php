...
function checkStockExists($request, $id)
{
    ...
    })->when($request->sph != '' && $request->cyl != '', function ($q) use ($spherical, $sph, $cyl) {
        return $q->whereRaw("IF($spherical, CAST($spherical AS DECIMAL(4,2)) = CAST(sph AS DECIMAL(4,2))+CAST(cyl AS DECIMAL(4,2)), 1)")->orWhereIn('sph', $sph)->orWhereIn('cyl', $cyl);
